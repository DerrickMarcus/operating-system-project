...
pid_t Sorter::fork_and_sort(int left, int right)
{
    ...
    {
        // parent process
        this->_active_procs_num.fetch_add(1);
        int status = 0;
        ::waitpid(pid, &status, 0);
        this->_active_procs_num.fetch_sub(1);
    }
    return pid;
}
...
void Sorter::quick_sort(int left, int right)
{
    ...
    if (this->_active_procs_num.load() <= this->_max_procs_num)
    {
        pid_t pid = this->fork_and_sort(pivot + 1, right);
        if (pid > 0)
        {
            // The parent process only sort the left part.
