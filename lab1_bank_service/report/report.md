...
本程序中时间的单位均为毫秒(ms)，由于线程运行指令的速度极快，所以程序中顾客线程和柜员线程中会在不同状态下，对前一条指令或后一条指令进行时间点捕获，存在一些误差，但是在毫秒量级上对精度几乎没有影响。
...
答：锁变量。轮转法。Peterson 算法。硬件方法——禁止中断。硬件指令方法。
